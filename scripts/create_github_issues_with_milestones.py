#!/usr/bin/env python3
"""
GitHub Issues Creator for Kisan Saathi Project
Creates all issues with milestones for sprint tracking
"""
...
# Sprint definitions with milestones
sprints = {
    "Sprint 1": {
        "title": "🚀 Sprint 1: Infrastructure Setup & Data Collection",
        "description": "Set up AWS EC2 GPU instance and collect Gujarati agricultural data",
        "due_date": "2024-02-15",  # 2 weeks from start
        "tasks": [
            {
...
`data`, `qa`, `instruction-tuning`, `gujarati`, `sprint-1`""",
    ...
    "Sprint 2": {
        "title": "📊 Sprint 2: Data Processing & Knowledge Base Creation",
...
`data`, `splits`, `training`, `sprint-2`""",
    ...
    "Sprint 3": {
        "title": "🧠 Sprint 3: Base Model Selection & Baseline",
...
`training`, `lora`, `configuration`, `sprint-3`""",
    ...
    "Sprint 4": {
        "title": "🎯 Sprint 4: LoRA Fine-tuning",
...
`evaluation`, `metrics`, `human-eval`, `sprint-4`""",
    ...
    "Sprint 5": {
        "title": "🔍 Sprint 5: RAG Pipeline Implementation",
...
`evaluation`, `rag`, `metrics`, `sprint-5`""",
    ...
    "Sprint 6": {
        "title": "⚡ Sprint 6: Model Quantization & Mobile Optimization",
...
`deployment`, `packaging`, `mobile`, `sprint-6`""",
    ...
    "Sprint 7": {
        "title": "🌾 Sprint 7: CNN for Crop Disease Detection",
...
`evaluation`, `cnn`, `metrics`, `confusion-matrix`, `sprint-7`""",
    ...
    "Sprint 8": {
        "title": "🔗 Sprint 8: Multi-modal Integration & Final Testing",
...
def create_milestone(sprint_name, sprint_data):
    """Create a milestone for a sprint"""
    milestone_data = {
        "title": sprint_data["title"],
        "description": sprint_data["description"],
        "due_on": sprint_data["due_date"] + "T23:59:59Z",
        "state": "open"
    }
    ...
    response = requests.post(MILESTONES_URL, json=milestone_data, headers=headers)
    
    if response.status_code == 201:
        milestone = response.json()
        print(f"✅ Created milestone: {sprint_data['title']}")
        return milestone["number"]
    else:
        print(f"❌ Failed to create milestone {sprint_name}: {response.text}")
        return None

def create_issue(task, milestone_number, sprint_name):
...
def main():
    """Main function to create all milestones and issues"""
    
    if GITHUB_TOKEN == "YOUR_GITHUB_TOKEN":
        print("❌ Please set your GitHub token in the script!")
        print("1. Go to GitHub Settings → Developer settings → Personal access tokens")
        print("2. Generate a new token with 'repo' permissions")
        print("3. Replace 'YOUR_GITHUB_TOKEN' in the script")
        return
    
    print("🚀 Creating GitHub Milestones and Issues for Kisan Saathi")
    print("=" * 60)
    ...
    for sprint_name, sprint_data in sprints.items():
        milestone_number = create_milestone(sprint_name, sprint_data)
        if milestone_number:
            milestones[sprint_name] = milestone_number
    ...
    total_issues = 0
    for sprint_name, sprint_data in sprints.items():
        if sprint_name in milestones:
            print(f"\n🎯 Creating issues for {sprint_data['title']}")
            milestone_number = milestones[sprint_name]
            
            for task in sprint_data["tasks"]:
                issue_number = create_issue(task, milestone_number, sprint_name)
                if issue_number:
                    total_issues += 1
                time.sleep(1)  # Rate limiting
    
    print(f"\n🎉 Successfully created {total_issues} issues across {len(milestones)} milestones!")
