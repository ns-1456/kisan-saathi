...
# GitHub Issues Creator with Milestones for Kisan Saathi
# This script creates all milestones and issues using GitHub CLI

echo "🚀 Creating GitHub Milestones and Issues for Kisan Saathi"
echo "=========================================================="
...
gh api repos/ns-1456/kisan-saathi/milestones -f title="🚀 Sprint 1: Infrastructure Setup & Data Collection" -f description="Set up AWS EC2 GPU instance and collect Gujarati agricultural data" -f due_on="2024-02-15T23:59:59Z"

# Sprint 2: Data Processing & Knowledge Base Creation (Weeks 3-4)
gh api repos/ns-1456/kisan-saathi/milestones -f title="📊 Sprint 2: Data Processing & Knowledge Base Creation" -f description="Clean data, create structured knowledge base, and build vector embeddings" -f due_on="2024-03-01T23:59:59Z"

# Sprint 3: Base Model Selection & Baseline (Weeks 5-6)
gh api repos/ns-1456/kisan-saathi/milestones -f title="🧠 Sprint 3: Base Model Selection & Baseline" -f description="Evaluate and select base SLM, establish baseline performance" -f due_on="2024-03-15T23:59:59Z"

# Sprint 4: LoRA Fine-tuning (Weeks 7-8)
gh api repos/ns-1456/kisan-saathi/milestones -f title="🎯 Sprint 4: LoRA Fine-tuning" -f description="Fine-tune selected SLM using LoRA on Gujarati agricultural corpus" -f due_on="2024-03-29T23:59:59Z"

# Sprint 5: RAG Pipeline Implementation (Weeks 9-10)
gh api repos/ns-1456/kisan-saathi/milestones -f title="🔍 Sprint 5: RAG Pipeline Implementation" -f description="Build retrieval-augmented generation system with LangChain" -f due_on="2024-04-12T23:59:59Z"

# Sprint 6: Model Quantization & Mobile Optimization (Weeks 11-12)
gh api repos/ns-1456/kisan-saathi/milestones -f title="⚡ Sprint 6: Model Quantization & Mobile Optimization" -f description="Quantize models and optimize for mobile deployment" -f due_on="2024-04-26T23:59:59Z"

# Sprint 7: CNN for Crop Disease Detection (Weeks 13-14)
gh api repos/ns-1456/kisan-saathi/milestones -f title="🌾 Sprint 7: CNN for Crop Disease Detection" -f description="Train lightweight CNN for crop disease classification" -f due_on="2024-05-10T23:59:59Z"

# Sprint 8: Multi-modal Integration & Final Testing (Weeks 15-16)
gh api repos/ns-1456/kisan-saathi/milestones -f title="🔗 Sprint 8: Multi-modal Integration & Final Testing" -f description="Integrate all components and comprehensive testing" -f due_on="2024-05-24T23:59:59Z"
...
### Dependencies
None" \
  --label "infrastructure,aws,gpu,sprint-1" \
...
AWS EC2 instance setup" \
  --label "infrastructure,python,dependencies,sprint-1" \
  --milestone "$MILESTONE_1"
...
Python environment setup" \
  --label "data,gujarati,indiccorp,sprint-1" \
  --milestone "$MILESTONE_1"
...
### Dependencies
Python environment setup" \
  --label "data,scraping,agriculture,gujarati,sprint-1" \
  --milestone "$MILESTONE_1"
...
Government resource collection" \
  --label "data,qa,instruction-tuning,gujarati,sprint-1" \
  --milestone "$MILESTONE_1"
...
IndicCorp dataset processing" \
  --label "data,preprocessing,gujarati,sprint-2" \
...
### Dependencies
Government resource collection" \
  --label "data,structuring,json,agriculture,sprint-2" \
  --milestone "$MILESTONE_2"
...
Document structuring" \
  --label "rag,embeddings,faiss,vector-store,sprint-2" \
  --milestone "$MILESTONE_2"
...
Q&A dataset creation" \
  --label "data,splits,training,sprint-2" \
  --milestone "$MILESTONE_2"
...
Dataset splits creation" \
  --label "models,evaluation,gujarati,sprint-3" \
...
### Dependencies
SLM evaluation" \
  --label "data,training,lora,sprint-3" \
  --milestone "$MILESTONE_3"
...
Training data preparation" \
  --label "training,lora,configuration,sprint-3" \
  --milestone "$MILESTONE_3"
...
echo ""
echo "🎯 Creating remaining sprint issues..."
...
### Dependencies
LoRA configuration" \
  --label "training,lora,domain-adaptation,sprint-4" \
...
Domain adaptation training" \
  --label "training,instruction-tuning,qa,sprint-4" \
  --milestone "$MILESTONE_4"
...
Instruction fine-tuning" \
  --label "evaluation,metrics,human-eval,sprint-4" \
  --milestone "$MILESTONE_4"
...
Vector store creation, LoRA fine-tuning" \
  --label "rag,langchain,pipeline,sprint-5" \
...
RAG pipeline implementation" \
  --label "rag,optimization,prompting,sprint-5" \
  --milestone "$MILESTONE_5"
...
### Dependencies
RAG optimization" \
  --label "evaluation,rag,metrics,sprint-5" \
  --milestone "$MILESTONE_5"
...
### Dependencies
LoRA fine-tuning" \
  --label "quantization,mobile,optimization,sprint-6" \
...
### Dependencies
Model quantization" \
  --label "mobile,conversion,onnx,tflite,sprint-6" \
  --milestone "$MILESTONE_6"
...
Mobile format conversion" \
  --label "deployment,packaging,mobile,sprint-6" \
  --milestone "$MILESTONE_6"
...
### Dependencies
None" \
  --label "data,images,cnn,plantvillage,sprint-7" \
...
Image dataset preparation" \
  --label "cnn,training,mobilenet,disease-detection,sprint-7" \
  --milestone "$MILESTONE_7"
...
### Dependencies
CNN training" \
  --label "evaluation,cnn,metrics,confusion-matrix,sprint-7" \
  --milestone "$MILESTONE_7"
...
RAG pipeline, CNN training" \
  --label "integration,multimodal,unified-system,sprint-8" \
...
Multi-modal integration" \
  --label "testing,performance,optimization,sprint-8" \
  --milestone "$MILESTONE_8"
...
### Dependencies
System testing" \
  --label "documentation,deployment,api,sprint-8" \
  --milestone "$MILESTONE_8"
